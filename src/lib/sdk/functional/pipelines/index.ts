 ...
 * @nestia Generated by Nestia - https://github.com/samchon/nestia
 */
export async function deletePipeline(connection: IConnection, pipelineId: string): Promise<void> {
  return PlainFetcher.fetch(connection, {
 ...
 * @summary 배포 상태 조회
 * @throws 401 인증 실패
 * @throws 404 배포 정보를 찾을 수 없음
 * @controller PipelineController.getDeploymentStatus
 * @path GET /pipelines/:pipelineId/deployment/status
 * @accessor api.functional.pipelines.getDeploymentStatus
 * @nestia Generated by Nestia - https://github.com/samchon/nestia
 */
export async function getDeploymentStatus(
  connection: IConnection,
  pipelineId: string
): Promise<getDeploymentStatus.Output> {
  return PlainFetcher.fetch(connection, {
    ...getDeploymentStatus.METADATA,
    template: getDeploymentStatus.METADATA.path,
    path: getDeploymentStatus.path(pipelineId)
  });
}
export namespace getDeploymentStatus {
  export type Output = {
    status: string;
  ...
  export const METADATA = {
    method: 'GET',
    path: '/pipelines/:pipelineId/deployment/status',
    request: null,
    response: {
      type: 'application/json',
      encrypted: false
  ...
  export const path = (pipelineId: string) =>
    `/pipelines/${encodeURIComponent(pipelineId?.toString() ?? 'null')}/deployment/status`;
}
