<script lang="ts" generics="T">
  ...
  const visibleEnd = $derived(Math.min(items.length, Math.ceil((scrollTop + height) / itemHeight)));
...
>
  <div style="height: {totalHeight}px; position: relative;">
    {#each visibleItems as { item, index, top } (index)}
      <div style="position: absolute; top: {top}px; left: 0; right: 0; height: {itemHeight}px;">
